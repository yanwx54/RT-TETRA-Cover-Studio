...
> 当前状态：V1.0 四类自动端到端验收和 Python 3.13.5 Windows onedir 构建已完成，待 Word 视觉复核与现场校准。
...
- 最小计算闭环；
- ITU Indoor、Tunnel、低频地面、完整 COST231-WI 和高架标定模型；
- 默认配置；
- 四类标准算例；
- 计算结果序列化接口；
- 边界和异常测试；
...
- Python 3.13.5 onedir 程序启动 8 秒冒烟验证；
- 49 个核心、传播参考点、示例、GUI、报告导出和端到端验收测试；
- 四类标准算例 GUI 加载、计算、曲线和 Word/PDF 导出验收；
- 四份 PDF 共 20 页视觉检查及长文本表格换行修复。
...
当前项目已完成 COST231-WI 公式与范围修正、低频地面标定模型、隧道/高架显式校准参数和独立参考点测试。未经现场校准的默认工程参数会在 GUI 和报告中标记为初步估算。
...
7. [`docs/14_传播模型适用性与公式修正.md`](docs/14_传播模型适用性与公式修正.md)：查看当前模型修正任务、低频地面模型决策和验收标准。

`docs/` 目录当前已建立完整文档结构，传播模型修正任务已单独立项，后续按参考点测试先行实施。
...
1. 获取隧道、高架和低频地面现场数据，替换默认未校准参数。
2. 由无线专业人员复核 COST231-WI 参考点和各场景标定来源。
3. 在可用的 Word 或 LibreOffice 环境完成四类 Word 报告页面级视觉复核。
...
| 工程目录 | 已创建 |
| 源码 | 双向链路预算、完整 COST231-WI、低频地面和可追溯校准模型已实现 |
| 配置 | 默认参数和覆盖等级配置已完成 |
| 示例 | 四类标准算例已完成 |
| 序列化 | 计算结果可输出 GUI/报告复用数据字典 |
| GUI | W 功率输入、双向链路结果、五段详细过程、双曲线和导出菜单已完成 |
| 报告导出 | 四类 Word/PDF 结构检查通过，PDF 20 页视觉检查通过，Word 视觉复核待完成 |
| 打包 | Python 3.13.5 Windows onedir 构建完成，exe 启动 8 秒冒烟通过 |
| 测试 | 49 个核心、传播参考点、示例、GUI、报告导出和端到端验收测试已通过 |
| 可运行程序 | Python 3.13.5 onedir 已生成，49 项测试和启动 8 秒冒烟通过 |
| 远程仓库 | 已推送至 GitHub |
