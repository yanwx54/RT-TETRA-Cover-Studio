...
> 当前状态：V1.0 文档体系初稿完成，技术栈已确定，核心计算、标准算例和结果序列化接口已完成。
...
当前项目已有可测试的核心计算模块、四类传播模型初版、标准算例和结果序列化接口，但还没有 GUI 或可运行应用。下一步应扩展边界测试，并开始 GUI 原型。
...
1. 扩展测试用例：边界参数、异常输入、曲线数据、无有效覆盖、达到搜索上限。
2. 准备 GUI 原型：参数输入、场景切换、计算按钮、结果展示。
3. 接入曲线展示。
4. 再开发报告导出。
